...
async function sendEmailToBidder(bid, order) {
    const mailOptions = {
...
<body>
    <div class="container">
        ...
        <p>To contact the designer, you can use the following information:</p>
        <p>Email: ${bid.User.email}</p>
        <p>Phone: ${bid.User.phone}</p>
        <p>and pay the designer <strong>${bid.amount} INR</strong></p>
        <p>at <a href="http://localhost:3000/customer/pay?id=${order.id}&user=${order.userId}&amount=${bid.amount}">this link</a></p>
    </div>
</body>
</html>
        `
